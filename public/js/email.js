...
ClassicEditor
    ...
    .then(editor => {
        ...
        // Reapply when focus or blur occurs
        const doc = editor.editing.view.document;
        doc.on('focus', fixHeight);
        doc.on('blur', fixHeight);

        // (Optional) when content changes
        doc.on('change', fixHeight);
    })
    .catch(error => {
        console.error(error);
    });
